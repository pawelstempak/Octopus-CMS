...
if(isset($_POST['save']) or isset($_POST['save_and_close']))
{
	if($_GET['id']!="1")
	{
		...
		{
			$restful_url = restful_url($_POST['restful_url'],"second",$_GET['id']);
			$r_url = ", restful_url = '".$restful_url."'";
		}
		else
		{
			$param = $_POST['langid_'.$backend_language['id_language']];
			$restful_url = restful_url($param,"second",$_GET['id']);
			$r_url = ", restful_url = '".$restful_url."'";
		}
	}
	...
	if($_POST['parent_id']!=$par['parent_id'] and $_GET['id']!="1")
	{
			$stmt=$pdo->query("
						SELECT MAX(sort)
						FROM menus
						WHERE type = 'second' and parent_id = '".$_POST['parent_id']."'
						");
			$sor=$stmt->fetch();
			$stmt->closeCursor();
			unset($stmt);
	...
	$pdo -> exec("
				UPDATE menus
				SET type = 'second', status = '".$_POST['status']."', id_pages = '".$_POST['pages_id']."', parent_id = '".$parent_id."'".$sort.$r_url."
				WHERE id_menu = '".$_GET['id']."'
	");
	...
	CreateXMLSearchFile('second','menus');

	if(isset($_POST['save']) and $_POST['save']=="save_close"){header("Location: ".PATH_BACKEND."smenus/ok");}
	if(isset($_POST['save']) and $_POST['save']=="save"){header("Location: ".PATH_BACKEND."edit_smenus/ok/".$_GET['id']);}
}
#################################################################
...
$menus_list = CheckParentID($backend_language['id_language'],'second','menus');
...
$stmt=$pdo->query("
						...
						WHERE m.id_menu = l.id_element and m.type = 'second' and l.id_language = '".$backend_language['id_language']."' and m.id_menu = '".$_GET['id']."'
						");
$u=$stmt->fetch();
$stmt->closeCursor();
unset($stmt);
