							...
							<div class="box select-box-pad">
								...
								<div id="{$first_tab.code}">
									...
									<div class="left bold paddingl10" style="padding-top:2px;">
										<img src="{$path}/images/flags/{$first_tab.code}.gif" />&nbsp;{$lang.pages_list_edit_language} {$l.title}
							   	</div>
							  <br /><br />
							  ...
							  <div class="row{if $title == "" && $frontend_language.code != $first_tab.code} hide_this{/if}" id="lang_{$first_tab.code}">
									  <div class="input-field">
								        <input placeholder="{$lang.pages_list_placeholder}" id="title_{$first_tab.code}" type="text" name="langid_{$first_tab.id_language}" class="validate" value="{$title}" required{if $title == "" && $smarty.get.page=="edit_pages"} disabled{elseif $frontend_language.code != $first_tab.code && $smarty.get.page == "create_pages"} disabled{/if}>
								        <label for="title_{$first_tab.code}">{$lang.pages_list_page_title}</label>
								     </div>
									 <div class="input-field">
										 <ul>
										 ...
										 <li>
									  	 ...
									  	 {foreach item=u from=$pages_list}
											 {if $first_tab.id_language==$u.id_language}
											 <li>
							...
							<div id="{$l.code}">
								<div class="left">
									<div class="switch">
										<label>
											{$lang.pages_list_menus_off}
											<input type="checkbox" onclick="checkbox_hs('element_{$l.code}','lang_{$l.code}','title_{$l.code}','ctitle_{$l.code}')" id="element_{$l.code}"{if $title != ""} checked{elseif $frontend_settings.code == $l.code} checked{/if}>
											<span class="lever"></span>
											{$lang.pages_list_menus_on}
										</label>
									</div>
							  ...
							  <div class="row{if $title == "" && $frontend_language.code != $l.code} hide_this{/if}" id="lang_{$l.code}">
									  <div class="input-field">
								        <input placeholder="{$lang.pages_list_placeholder}" id="title_{$l.code}" type="text" name="langid_{$l.id_language}" class="validate" value="{$title}" required{if $title == "" && $smarty.get.page=="edit_pages"} disabled{elseif $frontend_language.code != $l.code && $smarty.get.page == "create_pages"} disabled{/if}>
